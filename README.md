...
| `dcs-endpoints`   | `VIP_DCS_ENDPOINTS`   | no        | `http://10.10.11.1:2379`    | A url that defines where to reach the DCS or Patroni REST API. Multiple endpoints can be passed to the flag or env variable using a comma-separated-list. In the config file, a list can be specified, see the sample config for an example. With `dcs-type=etcd` and `dcs-type=consul` an endpoint that cannot be reached is skipped in favour of the next one. With `dcs-type=patroni` only the first endpoint is used, since it has to be the REST API of this very node - asking another node would answer whether *that* node is the leader. Defaults to `http://127.0.0.1:2379` for `dcs-type=etcd`, `http://127.0.0.1:8500` for `dcs-type=consul` and `http://127.0.0.1:8008` for `dcs-type=patroni`. |
