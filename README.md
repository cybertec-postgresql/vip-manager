...
| `netmask`         | `VIP_NETMASK`         | yes       | `24`                        | The netmask that is associated with the subnet that the virtual IP `ip` is part of. For IPv4 a value of `-1` or less selects the default mask of the address class, an IPv6 address needs an explicit value between 1 and 128. |
